...
| `generateAssignmentDraft` | `{ classroomId, prompt?, file? }` | editable assignment draft with questions, answers, and approved hints |
| `synthesizeSpeech` | `{ text, speed }` | base64 MP3 generated by `gpt-4o-mini-tts` for an authenticated user |
...
The recommendation, audit, IEP, and assignment adapters use the OpenAI Responses API
structured-output parser, bounded output tokens, no retry, and `store: false`. Assignment drafting
defaults to `gpt-5.6-luna`; read-aloud uses the Audio API with `gpt-4o-mini-tts`. There is no Gemini
provider or browser AI client.

```text
AI_PROVIDER=fake       default for deterministic emulator workflows
...
OPENAI_IEP_MODEL             optional server override
OPENAI_ASSIGNMENT_MODEL      optional server override
```
...
The teacher-facing Responses operations share a transactionally enforced per-teacher quota of 5
calls per minute and 50 calls per UTC day. The emulator and fake providers bypass the production
kill switch and live-call quota. Read-aloud is authenticated separately and can use a fresh key from
the ignored emulator secret file. A disabled switch, exhausted quota, provider fallback, or unsafe
output leaves the teacher in the stable manual workflow.
