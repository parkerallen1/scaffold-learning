...
// Uploaded media is always teacher-selected and never generated by the model.
// The transport asks only for starter copy; the domain normalization adds an
// empty media collection before the recommendation reaches the approval UI.
const openAiInterestRewardSettingsSchema = z
  .object({
...
const openAiSupportSettingsSchema = z.discriminatedUnion('supportKey', [
  ...
  breakPromptSettingsSchema,
  dyslexiaFontSettingsSchema,
  openAiInterestRewardSettingsSchema,
]);
...
const normalizeOpenAiStructuredResult = (
  ...
  return {
    ...
    recommendations: rawResult.recommendations.map((recommendation) => {
      if (recommendation.proposedSettings.supportKey === 'interestReward') {
        return {
          ...recommendation,
          proposedSettings: { ...recommendation.proposedSettings, rewardMedia: [] },
        };
      }
